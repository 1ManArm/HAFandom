...
namespace Fandom
{
    internal class DBWork
    {
        ...
        public static void ExportData(string exportPath, string query = "SELECT * FROM Mechanics;")
        {
            using (SQLiteConnection conn = new SQLiteConnection(path))
            {
                ...
                while (reader.Read())
                {
                    for (int i = 0; i < reader.FieldCount; i++)
                    {
                        csvData.Append(reader[i].ToString().Replace(",", ";"));
                        if (i < reader.FieldCount - 1)
                            csvData.Append(",");
                    }
                    csvData.AppendLine();
